...
//accordion
$(".accord1").tabs(".accord1%20div.pane", {tabs: 'h2', effect: 'slide', initialIndex: null});
$(".accord2").tabs(".accord2%20div.pane", {tabs: 'h2', effect: 'fade', initialIndex: null});
...
function msieversion(){
 ...
 if ( msieversion() >= 9 )
      ...
      $(".accord1").tabs(".accord1%20div.pane", {tabs: 'h2', effect: 'fade', initialIndex: null});
...
$.fn.nivoSlider.defaults = {
	...
	controlNavThumbsReplace: '_thumb.jpg.htm',
	keyboardNav: true,
	pauseOnHover: true,
	manualAdvance: false,
	captionOpacity: 1,
...
{
var dataString='news_mail='+news_mail;$.ajax({type:"POST",url:"php/newsletter.php.htm",data:dataString,success:function(){$('#form-newsletter').html("<div id='news_message'></div>");$('#news_message').html("<h3>That was easy!</h3>").append("<p>We'll keep you updated on new versions, updates and latest news. </p>").hide().fadeIn(1500,function(){$('#news_message').append("<img id='mailsent' src='images/mail_sent.png' />");});}});
}
return false;});});
...
//image preloader
jQuery.preloadCssImages=function(a){function i(b,c){var e=false,f=[],g=[],j;var k=b.length;while(k--){var l="";if(c&&c[k]){j=c[k]}else{var m=b[k].href?b[k].href:"window.location.href";var n=m.split("/");n.pop();j=n.join("default.htm");if(j){j+="default.htm"}}var o=l.match(/[^\("]+\.(gif|jpg|jpeg|png)/g);if(o){var p=o.length;while(p--){var q=o[p].charAt(0)=="/"||o[p].match(":..//default.htm")?o[p]:j+o[p];if(jQuery.inArray(q,d)==-1){d.push(q)}}}if(!e&&b[k].imports&&b[k].imports.length){for(var r=0,s=b[k].imports.length;r<s;r++){var t=b[k].imports[r].href;t=t.split("..//default.htm");t.pop();t=t.join("..//default.htm");if(t){t+="..//default.htm"}var u=t.charAt(0)=="default.htm"||t.match("://")?t:j+t;g.push(u);f.push(b[k].imports[r])}}}if(f.length){i(f,g);return false}var v=a.simultaneousCacheLoading;while(v--){setTimeout(h,v)}}function h(){if(d&&d.length&&d[c]){var b=new Image;b.src=d[c];if(!b.complete){jQuery(b).bind("error load onreadystatechange",g)}else{g()}f=setTimeout(g,a.errorDelay)}}function g(){clearTimeout(f);if(d&&d.length&&d[c]){c++;if(a.statusTextEl){var b=d[c]?"Now Loading: <span>"+d[c].split("/")[d[c].split("/").length-1]:"Loading complete";jQuery(a.statusTextEl).html('<span class="numLoaded">'+c+'</span> of <span class="numTotal">'+d.length+'</span> loaded (<span class="percentLoaded">'+(c/d.length*100).toFixed(0)+'%</span>) <span class="currentImg">'+b+"</span></span>")}if(a.statusBarEl){var e=jQuery(a.statusBarEl).width();jQuery(a.statusBarEl).css("background-position",-(e-(e*c/d.length).toFixed(0))+"px 50%")}h()}}a=jQuery.extend({statusTextEl:null,statusBarEl:null,errorDelay:999,simultaneousCacheLoading:2},a);var b=[],c=0,d=[],e,f;i(document.styleSheets);return d};
...
jQuery(function() {
    ...
    // lava lamp
    var links = $('.menu>li>a,%20.menu>li>a span', '#menu').css({background: 'none'});
...
(function($){
	...
	var jqTransformPreloadHoverFocusImg = function(strImgUrl) {
		...
		strImgUrl = strImgUrl.replace(/^url\((.*)\)/,'$1').replace(/^\"%28._2a%29\"$/,'$1');
